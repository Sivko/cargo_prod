    ...
    <TouchableHighlight
      ...
      <View style={styles.card}>
        ...
        <View style={styles.info}>
          ...
          <Text>
            Габариты: {el.item?.data?.attributes?.customs[fields.length] ?? 'Д'}{' '}
            х {el.item?.data?.attributes?.customs[fields.width] ?? 'Ш'} х{' '}
            {el.item?.data?.attributes?.customs[fields.height] ?? 'В'}
          </Text>
          <Text>
            Вес: {el.item?.data?.attributes?.customs[fields.weight] ?? '_'} кг
          </Text>
          <Text>
            Транспорт: {el.item?.data?.attributes?.customs[fields.transport]}
          </Text>
          <Text>ШК: {el.item?.data?.attributes?.customs[fields.barcode]}</Text>
          {el.item?.data?.attributes?.customs[fields.scanTSD] === 'Найдено' && (
            <Text>
              Статус: <Text style={{color: 'green'}}>Найдено</Text>
            </Text>
          )}
          {el.item?.data?.attributes?.customs[fields.scanTSD] === 'Ошибка' && (
            <Text>
