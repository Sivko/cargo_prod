...
export interface IItemDocumentList {
  document: TDocuments;
  deleteDoc(item: TDocuments, index: number): void;
  index: number;
}

export const ItemDocumentList: FC<IItemDocumentList> = ({
  document,
  deleteDoc,
  index = 0,
}) => {
  ...
  function cropNameFile(name: string) {
    if (name.length > 14) {
      const sectionFileName = name.split('.');
      const startSection = sectionFileName[0].substr(0, 20);
      return `${startSection}...${sectionFileName[1]}`;
    }
